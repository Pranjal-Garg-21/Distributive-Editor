...
void do_write(const char* username, const char* filename, int initial_sentence_num) {
    ...
    while(1) {
        ...
        *first_space = '\0';
        ...
        // As per your spec, we don't wait for a response here
    }
...
void do_delete(const char* username, const char* filename) {
    ...
    if (ss_sock_fd < 0) {
        ...
        // Note: At this point, the NM has deleted the metadata, but the SS still
        // has the file. This is an "orphaned file" situation, which is acceptable
        // for this project, but in a real system would require a cleanup process.
        return;
    }
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        } else if (strcmp(command, "DELETE") == 0) { // UPDATED
            char* filename = strtok(NULL, " \n");
            if (filename == NULL) {
